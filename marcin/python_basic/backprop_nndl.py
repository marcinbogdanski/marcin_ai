...
class Network(object):
    ...
    def eval_err(self, data_vec):
        
        total_err = 0
        for x, y in data_vec:
            out = self.feedforward( x )
            total_err += np.sum(np.square(out - y))
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        for j in range(epochs):
            ...
            np.random.shuffle(training_data_temp)
            mini_batches = [
                training_data_temp[k:k+mini_batch_size]
