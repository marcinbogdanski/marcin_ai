...
from linear_vq import LinearEnvVQ
from agent_vq import AgentVQ

def test_run(nb_episodes, world_size, method,
            step_size, nb_steps=None, lmbda=None):
    env = LinearEnvVQ (world_size)  # 2x terminal states will be added internally
    agent = AgentVQ(world_size=world_size+2,    # add 2x terminal states
                action_space=2,
                step_size=step_size,
                nb_steps=nb_steps,
                lmbda=lmbda)  
    ...
    for e in range(nb_episodes):
        ...
        rms = np.sqrt(np.sum(np.power(
            LinearEnvVQ.GROUND_TRUTH[world_size] - agent.V, 2)) / world_size)
        RMSE.append(rms)
